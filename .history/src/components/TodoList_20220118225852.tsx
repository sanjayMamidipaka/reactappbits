...
export default function TodoList() {
  ...
  const [tagArray, setTagArray] = useState<{name1: string, index1: number}[]>([]);
    ...
    var handleCallBack = (childData: boolean, index: string) =>{
      console.log(index);
      // let newTagArray = tagArray.filter((tagObject) => tagObject.index1 !== index);
      todoListCompletedItems.forEach((value) => {
        const {idx} = value;
        if (index == idx) {
          consolelog(idx, index)
          setTodoListCompletedItems([...todoListCompletedItems, {title: title, dueDate: date, tagList: tagArray, completed: false, idx: new Date().getTime().toString(), checked: !checked}])
        }
