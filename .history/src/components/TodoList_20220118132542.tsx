...
export default function TodoList() {
  ...
  const [tagArray, setTagArray] = useState<Object[]>([]);
  ...
  <div className="card">
    <label htmlFor="nameLabel">Title </label>
        ...
        <div>
          {tagArray.map(({name1, index}: {name: string, index: number}) => {
            return <Tag name={name1} key={index} theIndex={index} remove={removeElement}></Tag>
          })}
        </div>
