...
export default function TodoList() {
  ...
  const [tagArray, setTagArray] = useState<{name1: string, index: number}[]>([]);
  ...
  const whenSubmitted = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const addNewTag = (e: React.MouseEvent<HTMLButtonElement>) => {
      e.preventDefault();
      if (tagValue !== "") {
        setTagArray((tagArray: {name1: string, index: number}[]) => {
          return [...tagArray, {name1: tagValue, index: index}];
        });
        setIndex(index + 1);
      }
  ...
  <div className="card">
    <label htmlFor="nameLabel">Title </label>
        ...
        <div>
          {tagArray.map((tagObject: {name1: string, index: number}) => {
            return <Tag name={tagObject.name1} key={index} theIndex={index} remove={()=>{}}></Tag>
          })}
        </div>
