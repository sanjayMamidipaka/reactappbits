...
export default function TodoList() {
  ...
  const [tagArray, setTagArray] = useState<{name1: string}[]>([]);

    const addNewTag = (e: React.MouseEvent<HTMLButtonElement>) => {
      e.preventDefault();
      if (tagValue !== "") {
        setTagArray((tagArray: {name1: string}[]) => {
          return [...tagArray, {name1: tagValue}];
        });
        console.log();
  ...
  <div className="card">
    <label htmlFor="nameLabel">Title </label>
        ...
        <div>
          {tagArray.map((tagObject: {name1: string}) => {
            return <Tag name={tagObject.name1} key={new Date().getTime().toString()} theIndex={new Date().getTime().toString()} remove={()=>{}}></Tag>
          })}
        </div>
