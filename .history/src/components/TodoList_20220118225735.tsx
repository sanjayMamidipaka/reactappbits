  ...
  const [tagArray, setTagArray] = useState<{name1: string, index1: number}[]>([]);
    ...
    var handleCallBack = (childData: boolean, index: string) =>{
      ...
      todoListCompletedItems.forEach((value) => {
        const {idx} = value;
        if (index == idx) {
          setTodoListCompletedItems([...todoListCompletedItems, {title: title, dueDate: date, tagList: tagArray, completed: !completed, idx: new Date().getTime().toString(), checked: checked}])
        }
