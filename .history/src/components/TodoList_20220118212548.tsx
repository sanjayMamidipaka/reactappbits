...
export default function TodoList() {
  ...
  const [tagArray, setTagArray] = useState<{name1: string, index1: number}[]>([]);
    ...
    const sortByTodo = () => {
      ...
      // if (todoToggle === true) {
      //   setTodoListCompletedItems.sort((a,b)=> {a.})
      // }
    ...
    <label htmlFor="nameLabel">Title </label>
      ...
      <div>
          {todoListCompletedItems.map((todoListItem) => {
            return <TodoItem todoListItem={todoListItem} key={todoListItem.idx} index={todoListItem.idx} setChecked={setChecked}>{checked}</TodoItem>
          })}
        </div>
